...
function downloadZip(url: string, dist: string): Promise<void> {
...
async function unzipFirstFileAsString(zipPath: string): Promise<string> {
...
(async () => {
  try {
    for (const f of files) {
      await downloadZip(f.url, f.path);
    }
