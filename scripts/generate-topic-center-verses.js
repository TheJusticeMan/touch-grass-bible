...
// node_modules/ml-pca/lib/pca.js
var require_pca = __commonJS({
  "node_modules/ml-pca/lib/pca.js"(exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.PCA = void 0;
    var ml_matrix_1 = require_matrix();
    var PCA2 = class _PCA {
      constructor(dataset, options = {}, model) {
        ...
        switch (method) {
          case "covarianceMatrix": {
            const covarianceMatrix = new ml_matrix_1.MatrixTransposeView(datasetMatrix).mmul(datasetMatrix).div(datasetMatrix.rows - 1);
...
// scripts/generate-topic-center-verses.ts
var import_ml_pca = __toESM(require_pca(), 1);
import { readFileSync, writeFileSync } from "fs";
var META_PATH = process.env.EMBED_META ?? "processing/bible-embeddings.ollama.qwen3-embedding-0-6b.meta.json";
var BIN_PATH = process.env.EMBED_BIN ?? "processing/bible-embeddings.ollama.qwen3-embedding-0-6b.bin";
var TRANSLATION_PATH = process.env.TRANSLATION_PATH ?? "data/translations/KJV.json";
var VERSE_TOPICS_PATH = process.env.VERSE_TOPICS_PATH ?? "dist/data/bible-verse-topics.json";
var OUTPUT_PATH = process.env.TOPIC_CENTERS_OUTPUT ?? "dist/data/bible-topic-centers.json";
var TOPIC_CENTER_COUNT = Math.max(1, parseInt(process.env.TOPIC_CENTER_COUNT ?? "5", 10));
var PCA_COMPONENTS = Math.max(2, parseInt(process.env.PCA_COMPONENTS ?? "64", 10));
var PCA_ENABLED = (process.env.PCA_ENABLED ?? "true").toLowerCase() !== "false";
function buildVerseIndex(bible) {
  const rows = [];
...
function loadMatrix(int16, rows, dims, scale) {
...
function validateTopicAssignments(verses, topics) {
  if (verses.length !== topics.length) {
    throw new Error(`Topic assignment count mismatch: ${topics.length} assignments for ${verses.length} verses`);
  }
  for (let i = 0; i < verses.length; i++) {
    const verse = verses[i];
    const topic = topics[i];
    if (verse.book !== topic.book || verse.chapter !== topic.chapter || verse.verse !== topic.verse) {
      throw new Error(
        `Topic assignment order mismatch at index ${i}: expected ${verse.book} ${verse.chapter}:${verse.verse}, got ${topic.book} ${topic.chapter}:${topic.verse}`
      );
    }
  }
}
async function run() {
  ...
  const scale = meta.quantScale ?? 1e4;
  if (meta.granularity === "chapter") {
    throw new Error("Topic center extraction expects verse-level embeddings, but meta granularity is chapter");
  }
  ...
  const bible = JSON.parse(readFileSync(TRANSLATION_PATH, "utf8"));
  const verses = buildVerseIndex(bible);
  const topics = JSON.parse(readFileSync(VERSE_TOPICS_PATH, "utf8"));
  validateTopicAssignments(verses, topics);
  console.log(`Model: ${meta.model}  |  dims: ${dims}  |  quant scale: ${scale}`);
  console.log(`Loaded ${verses.length} verse embeddings and ${topics.length} topic assignments`);
  const matrix = loadMatrix(int16, verses.length, dims, scale);
  let features = matrix;
  let pcaComponentsUsed = null;
  if (PCA_ENABLED) {
    ...
    if (pcaComponents < dims) {
      ...
      const pca = new import_ml_pca.PCA(matrix, { center: true, scale: false });
  ...
  const topicToIndices = /* @__PURE__ */ new Map();
  ...
  const outputTopics = [...topicToIndices.entries()].sort((left, right) => left[0] - right[0]).map(([topic, indices]) => {
    const centroid = new Array(featureDims).fill(0);
    for (const index of indices) {
      const feature = features[index];
      for (let d = 0; d < featureDims; d++) centroid[d] += feature[d];
    }
    for (let d = 0; d < featureDims; d++) centroid[d] /= indices.length;
    const centerVerses = indices.map((index) => ({
      ...verses[index],
      distance: squaredDistance(features[index], centroid)
    })).sort((left, right) => left.distance - right.distance).slice(0, TOPIC_CENTER_COUNT);
    return {
      topic,
      size: indices.length,
      centerVerses
  ...
  console.log(`Wrote ${output.topics.length} topic center sets -> ${OUTPUT_PATH}`);
}
run().catch((err) => {
