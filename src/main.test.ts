...
describe("deepMerge (via loadsettings behavior)", () => {
  ...
  test("DEFAULT_SETTINGS has aiApiKey as empty string", () => {
...
describe("deepMerge utility", () => {
  ...
  test("nested objects are deep merged (not replaced wholesale)", () => {
    ...
    const saved: Partial<typeof defaults> = { style: { font: "sans", size: 16, spacing: true } }
  ...
  test("deeply nested objects are recursively merged", () => {
    ...
    const saved: Partial<typeof defaults> = { outer: { inner: { x: 99, y: 2 } } }
